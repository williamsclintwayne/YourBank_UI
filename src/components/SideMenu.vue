<template>
  ...
  <div class="side-menu-overlay animate-fade-in" @click.self="$emit('close')" @keydown.esc="$emit('close')"
    tabindex="-1">
    <aside class="side-menu card-surface animate-slide-in" role="dialog" aria-label="Quick actions">
      ...
      <div class="menu-body">
        ...
        <nav class="menu-group">
          ...
          <button class="menu-item" @click="$emit('navigate', 'transactionHistory')">
            <span class="item-icon bg-indigo-500/10 text-indigo-600">🧾</span>
            <div class="item-text">
              <span class="item-title">Transaction History</span>
              <span class="item-sub">View all transaction history</span>
            </div>
          </button>

          <button class="menu-item" @click="$emit('navigate', 'updateProfile')">
            <span class="item-icon bg-amber-500/10 text-amber-600">✏️</span>
            <div class="item-text">
          ...
          <button class="menu-item" @click="$emit('navigate', 'viewProfile')">
            <span class="item-icon bg-teal-500/10 text-teal-600">👤</span>
            <div class="item-text">
              <span class="item-title">View Profile</span>
              <span class="item-sub">View your latest profile details</span>
            </div>
          </button>

          <button class="menu-item" @click="$emit('navigate', 'notificationSettings')">
            <span class="item-icon bg-rose-500/10 text-rose-600">🔔</span>
            <div class="item-text">
              <span class="item-title">Notification Settings</span>
              <span class="item-sub">Set up your notification preferences</span>
            </div>
          </button>
        </nav>
